...
let gtgVAT (n: int) (x: float) : float = x * (1.0 + float n / 100.0)
        ...
        async {
            ...
            | OrderDrink (customer, drink, quantity, paymentOption) ->
                ...
                let priceWithVAT = 
                    ...
                    | _        -> price
                printfn "Hello %s, please pay DKK%.2f for your %d %s drinks using %A. Thanks!" customer.Name priceWithVAT quantity drink.Name paymentOption
            | LeaveAComment comment ->
                printfn "Customer comment: %s" comment
